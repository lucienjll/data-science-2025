...
- Can you confidently distinguish between household incomes in Suffolk
  ...
  - No because given the range for the household income, household types
    are very close to each other. Furthermore, their upper and lower
    bounds are well within each other’s boundaries as well.
...
df_q7 <- 
  ...
  mutate(
    county = str_remove(geographic_area_name, " County,.*$"),
    county = fct_reorder(county, income_SE)
  )%>%
  ggplot(
    aes(
    x = county, 
...
```
    ...
    ## Warning: There was 1 warning in `mutate()`.
    ## ℹ In argument: `county = fct_reorder(county, income_SE)`.
    ## Caused by warning:
    ## ! `fct_reorder()` removing 2 missing values.
    ## ℹ Use `.na_rm = TRUE` to silence this message.
    ## ℹ Use `.na_rm = FALSE` to preserve NAs.
