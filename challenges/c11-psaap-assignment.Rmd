...
-   Which model is *most accurate*? Which is *least accurate*?
    -   fit_cheat has the lowest mean square error and the highest rsquare value, which means that the model is most accurate
    -   fit_nonphysical has the highest mean square error and the lowest rsquare value, which means that the model is least accurate
-   What *Category* of variable is `avg_T`? Why is it such an effective predictor?
    -   avg-T is an output variable. It is a good predictor because it T_norm is computed in terms of avg_T.
-   Would we have access to `avg_T` if we were trying to predict a *new* value of `T_norm`? Is `avg_T` a valid predictor?
    -   no because the avg_T and T_norm are both output variables. avg_T changes when T_norm changes.
-   What *Category* of variable is `idx`? Does it have any physical meaning?
    -   idx is a metadata variable. It doesn't have any physical meaning other than denominating simulation runs.
...
-   Which columns are excluded in the model formula above? What categories do these belong to? Why are these important quantities to leave out of the model?
    -   the idx, avg_q, avg_T, rms_T have all been excluded. These are output variables which are important to leave out because they aren't input variables.
...
-   How do these standard deviations relate to the regression coefficients for `x` and `T_f`?
    -   The large regression coefficient and the large standard deviation in x shows that x has a high impact on the regression model, but has a wider spread of possible values
    -   The small regression coefficients in T_f shows that T_f doesn't have much impact on the regression model
-   Note that literally *all* of the inputs above have *some* effect on the output `T_norm`; so they are all "significant" in that sense. What does this tell us about the limitations of statistical significance for interpreting regression coefficients?
    -   The statistical significance of the regression coefficients isn't directly related to how large the effects are compared to others.
...
-   Which model tends to be more accurate? How can you tell from this predicted-vs-actual plot?
    -   q4 tends to be more accurate because the points tend to follow the trend line better in q4
-   Which model tends to be *more confident* in its predictions? Put differently, which model has *narrower prediction intervals*?
    -   model x tends is more confident because we can see that the predicted points form form 4 narrower prediction intervals
...
-   How much do you trust your model? Why?
    -   The model has a 93% value, but the rsquare value is lower at 67%. However, I am comparing the 93% value to that of the 80% coverage value. Therefore I am able to trust my model as it goes over the 80% coverage.
-   What kind of interval---confidence or prediction---would you use for this task, and why?
    -   I would use a prediction interval because we want to understand what new values are going to be.
-   What fraction of validation cases lie within the intervals you predict? (NB. Make sure to calculate your intervals *based on the validation data*; don't just use one single interval!) How does this compare with `pr_level`?
    -   93% of the validation cases lie within the intervals. This was higher than pr_level.
-   What interval for `T_norm` would you recommend the design team to plan around?
    -   I would recommend the design team to plan around the higher end of T_norm, at around 0.46 and 2.30 since that seems to be the upper and lower intervals from the design.
-   Are there any other recommendations you would provide?
    -   To reduce further uncertainty, you can add more input variables to control. I would try to control the number of particles, since that seems to be something that is controllable, and not an input variable that we can't control but measure.
