...
``` r
##Finding the true value for material property
df_stang_long %>%
  summarize(mean(E))
```
...
- How does increasing the thickness relate to the Poisson’s ratio?
...
- I’m looking at my plot, and while the plot can be more indepth, with
  each thickness, there are groupings of Poisson’s ratio. An increased
  thickness resulted in a smaller estimate of the grouping. This may
  suggest that manufacturing processes may have affected the resulting
  measurments since the thicker metal could have resulted in less time
  being rolled.
...
- Does this graph support or contradict the claim above?
  - This graph contradicts the plot because we see that more material
    results in a lesser value of both Poisson’s ratio and Young’s
    modulus.
- Is this evidence *conclusive* one way or another?
  - I think this evidence isn’t enough to conclude whether Poisson’s
    Ratio can be determined a material property. From classes, we know
    that Poisson’s ratio is considered a material property, and I would
    like to look at the range of the measured Poisson’s ratio to
    determine whether the variation is significant or not (Poisson’s
    ratio is between 0 and 1, how significant is a .1 change?) I also
    would like to look into how the angle of rolling resulted in the
    measured Poisson’s ratio to see if the variation there aligns with
